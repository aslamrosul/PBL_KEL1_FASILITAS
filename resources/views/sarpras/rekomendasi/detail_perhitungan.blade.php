                ...
                <div class="col-md-6">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tr>
                            ...
                            <td>
                                <span
                                    class="badge bg-{{ $laporan->bobotPrioritas->bobot_kode == 'URG' ? 'danger' : ($laporan->bobotPrioritas->bobot_kode == 'HIGH' ? 'warning' : 'primary') }}">
                        ...
                        <tbody>
                            @foreach($kriteria as $k)
                                <tr>
                                    ...
                                    <td>{{ ucfirst($k->kriteria_jenis) }}</td>
                            ...
                            <table class="table table-bordered table-hover text-nowrap">
                                ...
                                <tbody>
                                    @foreach($matriks_normalisasi as $i => $row)
                                        <tr
                                            class="{{ $matriks_keputusan[$i]['laporan_id'] == $laporan->laporan_id ? 'bg-light' : '' }}">
                                        ...
                                        <tbody>
                                            @foreach($kriteria as $k)
                                                <tr>
                                                    ...
                                                    <td>{{ number_format($solusi_ideal_positif[strtolower($k->kriteria_kode)], 4) }}
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                            @foreach($kriteria as $k)
                                                <tr>
                                                    <td>{{ $k->kriteria_kode }}</td>
                                                    <td>{{ number_format($solusi_ideal_negatif[strtolower($k->kriteria_kode)], 4) }}
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                ...
                                <tbody>
                                    @foreach($hasil_akhir as $index => $row)
                                        <tr class="{{ $row['laporan_id'] == $laporan->laporan_id ? 'bg-light' : '' }}">
                                            ...
                                            <td>
                                                @php
                                                    $bobot = $bobot_prioritas->firstWhere(function ($b) use ($row) {
                                                        return $b->skor_min <= $row['skor_akhir'] && $b->skor_max >= $row['skor_akhir'];
                                                    });
                                                @endphp
                                                @if($bobot)
                                                    <span
                                                        class="badge bg-{{ $bobot->bobot_kode == 'URG' ? 'danger' : ($bobot->bobot_kode == 'HIGH' ? 'warning' : 'primary') }}">
                                                        {{ $bobot->bobot_nama }}
                                                    </span>
                                                @else
